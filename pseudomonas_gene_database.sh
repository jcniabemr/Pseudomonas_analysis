...
# header : qseqid	sseqid	%identity	length	mismatch	gapopen	qstart	qend	sstart	send	evalue	bitscore
# >40% query length 
...
file=/home/jconnell/pseudomonas/pseudomonas_syringae_additional_sequencing/effector_blast
mkdir -p $temp_loc
for x in $(ls $file); do 
	cat ${file}/${x}/* | awk '{print $1}' | sort | uniq -c | awk '{print $2}' > $temp_loc/f1/"$x".txt
	cat ${file}/${x}/* | awk '($7 >= $8/100*40)&&($3 >= 70){print $1}' | sort | uniq -c | awk '{print $2}' > $temp_loc/f2/"$x".txt
	cat ${file}/${x}/* | awk '($11 +0) < 1e-5 && ($3 >= 20)' > $temp_loc/f3/"$x".txt
done 
####Combine lists 
for i in {1..3}
do 
paste $(for x in $(ls $temp_loc/f"$i"); do echo $temp_loc/f"$i"/"$x"; done) >> $temp_loc/effector_list_"$i".txt 
done 
